...
actor SpeziHealthKitQueryStandard: Standard, EnvironmentAccessible, HealthKitConstraint, OnboardingConstraint, AccountStorageConstraint {
    enum SpeziHealthKitQueryStandardError: Error {
    ...
    private let logger = Logger(subsystem: "SpeziHealthKitQuery", category: "Standard")
    
    
    private var userDocumentReference: DocumentReference {
        get async throws {
            guard let details = await account.details else {
                throw SpeziHealthKitQueryStandardError.userNotAuthenticatedYet
            }
    ...
    init() {
        if !FeatureFlags.disableFirebase {
            _accountStorage = Dependency(wrappedValue: FirestoreAccountStorage(storeIn: SpeziHealthKitQueryStandard.userCollection))
        }
    }
