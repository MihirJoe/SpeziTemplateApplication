...
/// A `Scheduler` using the ``SpeziHealthKitQueryTaskContext`` to schedule and manage tasks and events in the
/// SpeziHealthKitQuery.
typealias SpeziHealthKitQueryScheduler = Scheduler<SpeziHealthKitQueryTaskContext>


extension SpeziHealthKitQueryScheduler {
    static var socialSupportTask: SpeziScheduler.Task<SpeziHealthKitQueryTaskContext> {
        ...
        return Task(
            ...
            notifications: true,
            context: SpeziHealthKitQueryTaskContext.questionnaire(Bundle.main.questionnaire(withName: "SocialSupportQuestionnaire"))
        )
    }

    /// Creates a default instance of the ``SpeziHealthKitQueryScheduler`` by scheduling the tasks listed below.
    convenience init() {
